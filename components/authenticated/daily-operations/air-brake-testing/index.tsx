...
import { updateAirBrakeDate } from '@/app/(authenticated)/(daily-operations)/air-brake/actions';
...
export default function AirBrakeTesting() {
    ...
    const {
        ...
        queryKey: ['coaches-for-air-brake', searchTerm],
        queryFn: async ({ pageParam = 1 }) => {
            const filters: any[] = [
                { column: 'pohby', operator: 'equals', value: 'PL' },
                { column: 'shopindt', operator: 'isNotEmpty' }
            ];
    ...
    const mutation = useMutation({
        mutationFn: async () => {
            const response = await updateAirBrakeDate({
                coachNos: selectedCoaches.map(c => c.coachno),
                airBrakeDate: airBrakeDate
            });
            if (!response.success) throw new Error(response.error);
            return response;
        },
        onSuccess: (data: any) => {
            toast.success(data.message || 'Air brake testing updated successfully');
            setSelectedCoaches([]);
            setAirBrakeDate(new Date());
            queryClient.invalidateQueries({ queryKey: ['coaches-for-air-brake'] });
        },
        onError: (error) => {
    ...
    const handleSave = () => {
        ...
        if (!airBrakeDate) {
            toast.error('Please select an air brake date');
            return;
        }
        mutation.mutate();
    };
    ...
    return (
        <div className="container mx-auto py-6 space-y-6">
            <div className="flex items-center justify-between">
                <h1 className="text-3xl font-bold tracking-tight">Air Brake Testing Update</h1>
            </div>
                ...
                <div className="md:col-span-1 space-y-6">
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium">Air Brake Date</label>
                                <Popover>
                                    <PopoverTrigger asChild>
                                        <Button
                                            ...
                                            className={cn(
                                                ...
                                                !airBrakeDate && "text-muted-foreground"
                                            )}
                                        >
                                            {airBrakeDate ? (
                                                format(airBrakeDate, "PPP")
                                            ) : (
                                    ...
                                    <PopoverContent className="w-auto p-0" align="start">
                                        <Calendar
                                            mode="single"
                                            selected={airBrakeDate}
                                            onSelect={setAirBrakeDate}
                                            disabled={(date) =>
                            ...
                            <Button
                                className="w-full"
                                onClick={handleSave}
                                disabled={selectedCoaches.length === 0 || !airBrakeDate || mutation.isPending}
                            >
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead>Old Air Brake Date</TableHead>
                                            <TableHead>New Air Brake Date</TableHead>
                                        ...
                                        {selectedCoaches.length === 0 ? (
                                            ...
                                            selectedCoaches.map((coach) => (
                                                <TableRow key={coach.coachno}>
                                                    ...
                                                    <TableCell>
                                                        {coach.airbkdt ? format(new Date(coach.airbkdt), 'dd/MM/yyyy') : '-'}
                                                    </TableCell>
                                                    <TableCell className="text-green-600 font-medium">
                                                        {airBrakeDate ? format(airBrakeDate, 'dd/MM/yyyy') : '-'}
                                                    </TableCell>
                                                    <TableCell>
                                                        <Button
                                                            variant="ghost"
