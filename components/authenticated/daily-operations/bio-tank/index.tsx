...
import { updateBioTankDates } from '@/app/(authenticated)/(daily-operations)/bio-tank/actions';
...
export default function BioTank() {
    ...
    const {
        ...
        queryKey: ['coaches-for-bio-tank', searchTerm],
    ...
    const mutation = useMutation({
        mutationFn: async (data: { coaches: string[], btundt?: Date, btlddt?: Date }) => {
            const response = await updateBioTankDates({
        ...
        onSuccess: (data: any) => {
            toast.success(data.message || 'Bio-tank dates updated successfully');
            setSelectedCoachesUnload([]);
            setSelectedCoachesLoad([]);
            setUnloadDate(undefined);
            setLoadDate(undefined);
            queryClient.invalidateQueries({ queryKey: ['coaches-for-bio-tank'] });
        },
        onError: (error) => {
    ...
    const handleSaveUnload = () => {
        ...
        mutation.mutate({ coaches: selectedCoachesUnload.map(c => c.coachno), btundt: adjustedDate });
    };
    ...
    const handleSaveLoad = () => {
        ...
        adjustedDate.setHours(12);
        mutation.mutate({ coaches: selectedCoachesLoad.map(c => c.coachno), btlddt: adjustedDate });
    };

    return (
        <div className="container mx-auto py-6 space-y-6">
            <div className="flex items-center justify-between">
                <h1 className="text-3xl font-bold tracking-tight">Bio-tank Update</h1>
            </div>

            <Tabs defaultValue="unload" className="w-full">
                <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="unload">Bio-tank Unload</TabsTrigger>
                    <TabsTrigger value="load">Bio-tank Load</TabsTrigger>
                </TabsList>
                        ...
                        <div className="md:col-span-1 space-y-6">
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium">Unload Date (btundt)</label>
                                        <Popover>
                                                ...
                                                {selectedCoachesUnload.length === 0 ? (
                                                    ...
                                                    selectedCoachesUnload.map((coach) => (
                                                        <TableRow key={coach.coachno}>
                                                            ...
                                                            <TableCell>
                                                                {coach.btundt ? format(new Date(coach.btundt), 'dd/MM/yyyy') : '-'}
                                                            </TableCell>
                        ...
                        <div className="md:col-span-1 space-y-6">
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium">Load Date (btlddt)</label>
                                        <Popover>
                                            <PopoverTrigger asChild>
                                                <Button
                                                    variant={"outline"}
                                                ...
                                                {selectedCoachesLoad.length === 0 ? (
                                                    ...
                                                    selectedCoachesLoad.map((coach) => (
                                                        <TableRow key={coach.coachno}>
                                                            ...
                                                            <TableCell>{coach.code}</TableCell>
                                                            <TableCell>
                                                                {coach.btlddt ? format(new Date(coach.btlddt), 'dd/MM/yyyy') : '-'}
                                                            </TableCell>
                                                            <TableCell className="text-green-600 font-medium">
